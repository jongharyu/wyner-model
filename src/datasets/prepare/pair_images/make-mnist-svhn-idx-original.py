...
if __name__ == '__main__':
    ...
    train_svhn = datasets.MNIST('../data', train=True, download=True, transform=tx)
    test_svhn = datasets.MNIST('../data', train=False, download=True, transform=tx)
    ...
    svhn_l, svhn_li = train_svhn.targets.sort()
    idx1, idx2 = rand_match_on_idx(mnist_l, mnist_li, svhn_l, svhn_li, max_d=max_d, dm=dm)
    print('len train idx:', len(idx1), len(idx2))
    torch.save(idx1, '../data/train-ms-mnist-idx0.pt')
    torch.save(idx2, '../data/train-ms-mnist-idx1.pt')

    mnist_l, mnist_li = test_mnist.targets.sort()
    svhn_l, svhn_li = test_svhn.targets.sort()
    idx1, idx2 = rand_match_on_idx(mnist_l, mnist_li, svhn_l, svhn_li, max_d=max_d, dm=dm)
    print('len test idx:', len(idx1), len(idx2))
    torch.save(idx1, '../data/test-ms-mnist-idx0.pt')
    torch.save(idx2, '../data/test-ms-mnist-idx1.pt')
