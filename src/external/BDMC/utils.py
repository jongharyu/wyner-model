...
def log_normal(x, mean, logvar):
    ...
    Returns:
        output: [B]
    """
    mean = mean + torch.zeros_like(x)
    logvar = logvar + torch.zeros_like(x)
    return -0.5 * (logvar + ((x - mean).pow(2) / torch.exp(logvar))).reshape(x.shape[0], -1).sum(1)


def log_uniform(x, a):
    """Uniform (-a1,a1) x (-a2,a2) x ... x (-aZ,aZ)

    Args:
        x: [B,Z]
        a: [B,Z] positive values

    Returns:
        output: [B]
    """
    # currently, we always assume that x
    assert torch.all(torch.abs(x) <= a)
    return -(np.log(2) + torch.log(a)).sum(1).to(x.device)


def log_normal_full_cov(x, *params):
...
def log_bernoulli(logit, target):
    ...
    loss = torch.sum(loss.reshape(loss.shape[0], -1), 1)

    return loss


def mean_squared_error(prediction, target):
    prediction, target = F.flatten(prediction), F.flatten(target)
