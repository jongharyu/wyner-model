...
class VAE(nn.Module):
  ...
  def sample(self, mu, logvar):
    ...
    logqz = generic.log_normal(z, mu, logvar)

    zeros = torch.zeros(z.size()).cuda()
    logpz = generic.log_normal(z, zeros, zeros)
  ...
  def forward(self, x, k=1, warmup_const=1.):
    ...
    logpx = generic.log_bernoulli(x_logits, x)
    elbo = logpx + logpz - warmup_const * logqz

    # need correction for Tensor.repeat
    elbo = generic.log_mean_exp(elbo.view(k, -1).transpose(0, 1))
